...
const DropdownNotification = () => {
  ...
  const { data: allNotices } = useFetch([KY.notification], `${KY.notification}`, {}, 100000);
  ...
  return (
    <div className="relative">
      ...
      >
        <span
            className={`absolute -top-0.5 right-0 z-1 flex h-5 w-5 items-center justify-center rounded-full bg-meta-1 text-xs text-white ${notifying ? 'inline' : 'hidden'
            }`}
        >
          {count!=null && count}
        </span>
...
export function NotificationList({notice, path, notifying}: { notice: any, path: string, notifying:boolean }) {
  return <li  className=" relative flex flex-row items-center justify-center bg-gray-2 mx-2 px-4  dark:border-form-input dark:bg-meta-4">
      ...
      <Link
          className="flex flex-col gap-2.5 border-t border-stroke w-full  px-4.5 py-3 dark:border-form-input dark:bg-meta-4"
          href={` `}
          onClick={() => {
