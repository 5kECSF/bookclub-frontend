...
export function BorrowAction({row }: { row: IBorrow }) {
    ...
    const {  data: donation } = useFetch(
        ...
        `${KY.donation}`,{q: donationQ, bookId: row.bookId}
    );
    const {
        handleSubmit,
        register,
    ...
    const onSubmit = async (data: TAcceptDto) => {
       ...
       if(row.status===borrowStatus.WaitList){
           message="successfully accepted the borrowing request, now the book is reserved"
    ...
    const handleBorrowed= async ()=>{
        const resp = await makeReq(`${KY.borrow}/markTaken/${row._id}`, {}, MTD.POST);
        if (!resp.ok) return handleErr(resp.message);
        await queryClient.invalidateQueries({queryKey:[KY.borrow]})
        reset()
        toast.success(`successfully accepted the borrowing request, now the book is reserved`,);
    }
    ...
    return (
        <div>
            <div className="flex gap-2">
                ...
                {row.status===borrowStatus.WaitList&&<Button
                    variant={"outline"}
            ...
            >
                ...
                <div
                    className="w-inherit flex flex-col gap-9"
                >
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="p-6.5">
                            ...
                            {row.status===borrowStatus.Accepted&&<InputField
                                register={register}
                                errors={errors}
                            ...
                            />}
                            {row.status===borrowStatus.Taken&&<InputField
                                register={register}
                                errors={errors}
                                handleChange={handleChange}
                                label={"Returned Date"}
