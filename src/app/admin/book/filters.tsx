...
export function FilterDrawer({
  ...
  const { data: category } = useFetch([KY.category, ctgQ], `${KY.category}`, {
    limit: 100,
    q: ctgQ,
  });
  const [genQ, setGenQ] = useState("");
  const { data: genre } = useFetch([KY.genre, "100", genQ], `${KY.genre}`, {
    limit: 100,
    q: genQ,
  });
  const [authorQ, setAuthorQ] = useState("");
  const { data: author } = useFetch([KY.author, authorQ], `${KY.author}`, {
    limit: 100,
    q: authorQ,
  });
  return (
