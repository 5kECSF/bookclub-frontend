...
import {IDonation, TDonationDto, DonationValidator, bookStatusList} from "@/app/admin/donation/model-def";
...
const AddEdit = ({ isUpdate, isOpen, onClose, donation }: IBookProps) => {
    ...
    const onSubmit = async (data: IDonation) => {
        let resp: Resp<any>;
        if (isUpdate) {
            if (!donation || !("_id" in donation)) return handleErr("malformed update");
            resp = await makeReq(`${KY.donation}/${donation._id}`, modifiedData, MTD.PATCH);
