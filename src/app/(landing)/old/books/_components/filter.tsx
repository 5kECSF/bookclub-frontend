...
const App = () => {
  ...
  let categories = ctgData.map((cat: ICategory) => ({
    label: cat.name,
    value: cat._id,
  }));
  if (categories)
  ...
  let genres = genData?.map((genre: Igenre) => ({
    label: genre.name,
    value: genre.name,
  }));
  const language = [
      ...
      >
        <div className="flex flex-col items-start justify-between gap-4">
          ...
          <Select
            ...
            style={{ width: "100%" }}
            onChange={(value) =>
              setFilter((pre) => ({ ...pre, categoryId: value }))
            }
            options={categories}
          ...
          <Select
            ...
            style={{ width: "100%" }}
            onChange={(value) =>
              setFilter((pre) => ({ ...pre, genres: value }))
            }
            options={genres}
            value={filter.genres}
          />
          ...
          >
            {language.map((val, i) => {
              return (
                <Radio
                  key={i}
                  data-test="language-radio-button"
                  value={val.value}
                >
                  {val.label}
                </Radio>
          ...
          <Button
            ...
            onClick={handleClick}
            className="mt-auto text-white [background:linear-gradient(161.68deg,_#fa7c54,_#ec2c5a)]"
          >
            Search
          </Button>
        </div>
