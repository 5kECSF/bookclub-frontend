...
import { AiFillCheckCircle, AiFillStar, AiOutlineArrowRight } from 'react-icons/ai'
...
const BookCard2 = ({ book, category }: { book: IBook, category: string }) => {
    ...
    return (
        <div className='grid grid-cols-4 sm:grid-cols-5 md:grid-cols-5 lg:grid-cols-7 gap-10 h-[120px] items-center bg-white rounded px-4 md:px-0'>
            ...
            <div>
                <Button onClick={() => router.push(`/books/${book._id}`)} variant={'outline'} className='hidden sm:flex'>Preview</Button>
                <Button onClick={() => router.push(`/books/${book._id}`)} variant={'ghost'} className='sm:hidden p-8'> <AiOutlineArrowRight /></Button>
